...
class SignupViewModel(context: Application, private val dataRepository: DataRepository) :
    BaseAndroidViewModel(context) {

    var user: MutableLiveData<OneShotEvent<UserResponse>> = MutableLiveData()

    fun signUp(name: String, phone: String, email: String, password: String, birthday: String, address: String, city: String, country: String, avatar: String, token: String) {
        if(isOnline(getContext())) {
            dataRepository.userRegister(name, phone, email, password, birthday, address, city, country, avatar, token, object : UserDataSource.RegisterCallback {
                override fun onRegister(u: UserResponse) {
    ...
    fun signUpWithoutImage(name: String, phone: String, email: String, password: String, birthday: String, address: String, city: String, country: String, token: String) {
        if(isOnline(getContext())) {
            showProgressBar(true)
            dataRepository.registerWithoutImage(name, phone, email, password, birthday, address, city, country, token, object : UserDataSource.RegisterCallback {
                override fun onRegister(u: UserResponse) {
                    showProgressBar(false)
                    user.value = OneShotEvent(u)
                }
